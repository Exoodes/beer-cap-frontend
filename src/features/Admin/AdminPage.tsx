...
export function AdminPage() {
  ...
  useEffect(() => {
    if (editingBeer) {
      editBeerForm.setValues({
        ...
        beer_brand_id:
          (editingBeer.brand?.id || editingBeer.beer_brand?.id)?.toString() ||
          null,
  ...
  const createCountryMut = useMutation({
    mutationFn: (vals: { name: string; description: string }) =>
      createCountry(vals.name, vals.description),
    onSuccess: () => {
  ...
  const updateBrandMut = useMutation({
    mutationFn: (vals: { id: number; name: string }) =>
      updateBrand(vals.id, { name: vals.name }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["brands"] });
  ...
  return (
    <Container size="xl" py="xl">
      ...
      <Tabs defaultValue="maintenance">
        ...
        <Tabs.Panel value="maintenance">
          <Stack gap="md">
            <Alert
              ...
              color="orange"
            >
              AI tasks are resource intensive.
            </Alert>
            <Paper withBorder p="md" radius="md">
              <Group align="flex-end">
                <NumberInput
                  label="Augmentation Variations"
                  value={augCount}
            ...
            <Table.Tbody>
              {beers.map((beer) => (
                <Table.Tr key={beer.id}>
                  ...
                  <Table.Td>
                    {beer.brand?.name || beer.beer_brand?.name || "N/A"}
                  </Table.Td>
                  <Table.Td>
                    <Badge color="yellow">{beer.rating}/10</Badge>
                  </Table.Td>
                  <Table.Td>
                    <Group gap="xs">
                      <ActionIcon
                      ...
                      <ActionIcon
                        ...
                        color="red"
                        onClick={() =>
                          handleConfirmDelete("Beer", beer.name, () =>
        ...
        <Tabs.Panel value="brands">
          <Paper withBorder p="md" mb="xl" bg="gray.0">
            <form
              onSubmit={brandForm.onSubmit((v) =>
                createBrandMut.mutate(v.name),
              )}
            >
              <Group align="flex-end">
                <TextInput
                  ...
                  <Table.Td>
                    <Group gap="xs">
                      ...
                      <ActionIcon
                        ...
                        color="red"
                        onClick={() =>
                          handleConfirmDelete("Brand", brand.name, () =>
        ...
        <Tabs.Panel value="countries">
          <Paper withBorder p="md" mb="xl" bg="gray.0">
            <form
              onSubmit={countryForm.onSubmit((v) => createCountryMut.mutate(v))}
            >
              <Stack gap="xs">
                <TextInput
                  label="New Country"
                ...
                />
                <Button type="submit" loading={createCountryMut.isPending}>
                  Add Country
                </Button>
              </Stack>
            </form>
                  ...
                  <Table.Td>
                    <Group gap="xs">
                      ...
                      <ActionIcon
                        ...
                        color="red"
                        onClick={() =>
                          handleConfirmDelete("Country", c.name, () =>
                            deleteCountryMut.mutate(c.id),
                          )
                        }
                      >
                        <IconTrash size={16} />
      ...
      >
        <form
          onSubmit={editBeerForm.onSubmit((v) => {
        ...
        >
          <Stack>
            ...
            <NumberInput
              ...
              min={0}
              max={10}
              {...editBeerForm.getInputProps("rating")}
            />
            <Select
              label="Beer Brand"
            ...
            />
            <Button type="submit" loading={updateBeerMut.isPending}>
              Save
            </Button>
          </Stack>
        </form>
      ...
      >
        <form
          onSubmit={editBrandForm.onSubmit((v) => {
            if (editingBrand)
              updateBrandMut.mutate({ id: editingBrand.id, name: v.name });
          })}
        >
          <Stack>
            <TextInput
              label="Brand Name"
              {...editBrandForm.getInputProps("name")}
            />
            <Button type="submit" loading={updateBrandMut.isPending}>
              Save
            </Button>
          </Stack>
        </form>
      ...
      {/* EDIT COUNTRY MODAL */}
      <Modal
        opened={!!editingCountry}
        onClose={() => setEditingCountry(null)}
        title="Edit Country"
      >
        <form
          onSubmit={editCountryForm.onSubmit((v) => {
            if (editingCountry)
              updateCountryMut.mutate({
        ...
        >
          <Stack>
            <TextInput
              label="Country Name"
              {...editCountryForm.getInputProps("name")}
            />
            <Textarea
              label="Description"
              {...editCountryForm.getInputProps("description")}
            />
            <Button type="submit" loading={updateCountryMut.isPending}>
              Save
            </Button>
          </Stack>
        </form>
